...
const WebinarTechById = () => {
    ...
    const getTechById = async () => {
        if (!id) return;
        await axios.get(`${process.env.REACT_APP_API_GATEWAY}api/doapi?func=GetWebinarbyid&apikey=${process.env.REACT_APP_API_KEY}&id=${id}`)
    ...
    const handleSendComment = (e) => {
        e.preventDefault();
        e.stopPropagation();
        axios.get(`${process.env.REACT_APP_API_GATEWAY}api/doupdateapi?func=Uploalbiocomments&Techcommentername=${user_id}&Techcommentdescription=${userComment}&apikey=${process.env.REACT_APP_API_KEY}&&pccid=${id}&userid=${user_id}`)
            .then(function () {
                setUserComment("");
        ...
        <div class="container">
            <div class="row">
                ...
                <div class="col-lg-12">
                    <form action="">
                        <article>
                            <h1 class="post-title mb-4">{tech?.WebinarName}</h1>
                            <div class="d-flex align-items-center gap-4 flex-wrap">
                                <p class="post-details">{tech?.writer} </p>
                                <p class="post-details">زمان حدودی مطالعه:
                                    {tech?.TimeWebinar ? tech.TimeWebinar : "---"}
                                    دقیقه </p>
                            </div>
                            <img src={`${process.env.REACT_APP_FILEMANAGER}${tech?.WebinarImage}`} alt=""/>
                            <div dangerouslySetInnerHTML={{__html: decodeHtml(tech?.Describtion)}}/>
                        </article>
                        <div class="line"></div>
...
export default WebinarTechById;